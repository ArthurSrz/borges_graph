...
export default function QueryAnimationControls({
  ...
  return (
    <div className={`bg-datack-panel rounded-lg p-4 space-y-4 border border-datack-border ${className}`}>
      {/* Main Controls */}
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-10 h-10 rounded-full bg-datack-yellow text-datack-black hover:opacity-90 transition-colors"
          >
            {isPlaying ? (
          ...
          <button
            onClick={onReset}
            className="flex items-center justify-center w-8 h-8 rounded bg-datack-black text-datack-muted hover:bg-datack-black-hover transition-colors border border-datack-border"
          ...
          <div className="flex items-center space-x-1">
            <button
              ...
              className="flex items-center justify-center w-8 h-8 rounded bg-datack-black text-datack-muted hover:bg-datack-black-hover transition-colors border border-datack-border disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm text-datack-muted">Speed:</span>
          <select
            value={speed}
            onChange={(e) => handleSpeedChange(parseFloat(e.target.value))}
            className="text-sm bg-datack-black border border-datack-border rounded px-2 py-1 text-datack-light"
          >
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm text-datack-muted">
        ...
        <div
          className="relative h-6 bg-datack-black rounded cursor-pointer border border-datack-border"
        ...
        >
          ...
          {animationTimeline.map((phase, index) => {
            ...
            return (
              <div
                key={phase.phase}
                className={`absolute top-0 h-6 rounded transition-all ${
                  isCurrentPhase
                    ? 'bg-datack-yellow'
              ...
              >
                ...
                {isCurrentPhase && (
                  <div
                    className="absolute top-0 h-6 bg-datack-yellow/80 rounded transition-all"
                ...
                {/* Phase Label */}
                {phaseWidth > 15 && (
                  <div className={`absolute inset-0 flex items-center justify-center text-xs font-medium ${isCurrentPhase ? 'text-datack-black' : 'text-datack-light'}`}>
                    {phase.phase}
                  </div>
                )}
              </div>
          ...
          <div
            className="absolute top-0 w-1 h-6 bg-datack-light border-2 border-datack-black rounded-full transition-all z-10"
            style={{ left: `${overallProgress * 100}%`, transform: 'translateX(-50%)' }}
          />
        </div>

        {/* Phase Labels Below Timeline */}
        <div className="flex justify-between text-xs text-datack-muted">
          {animationTimeline.map((phase, index) => (
            <div
              key={phase.phase}
              className={`text-center cursor-pointer hover:text-datack-muted ${
                index === currentPhaseIndex ? 'font-medium text-datack-yellow' : ''
              }`}
      ...
      {/* Current Phase Info */}
      {currentPhaseIndex >= 0 && (
        <div className="bg-datack-black rounded-lg p-3 border border-datack-border">
          ...
          <div className="mt-2 w-full bg-datack-panel rounded-full h-2">
            <div
              className="bg-datack-yellow h-2 rounded-full transition-all duration-300"
              style={{ width: `${currentProgress * 100}%` }}
            />
          </div>
        </div>
